...
export default function ProductsAdmin() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const slug = formData.name.toLowerCase().replace(/[^a-z0-9а-я]+/gi, '-').replace(/^-+|-+$/g, '');
      const productData = {
        ...formData,
        slug,
        images: imageUrls,
                  ...
                  <tr key={product.id} className="border-b border-cream-dark hover:bg-cream/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-1.5 flex-wrap">
                        ...
                        {product.is_constructor_item && (
                          <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">Ягода</span>
                        )}
                      </div>
                    </td>
                    <td className="py-3 px-4">
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-5">
                ...
                <div className="flex gap-4 p-4 bg-white rounded-xl border border-cream-dark shadow-sm flex-wrap">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox" 
                      checked={formData.is_constructor_item} 
                      onChange={(e) => setFormData({ ...formData, is_constructor_item: e.target.checked })}
                      className="w-4 h-4 accent-blue-500" 
                    />
                    <span className="text-sm font-semibold text-choco">Ягода для конструктора</span>
                  </label>
                </div>
              </div>
