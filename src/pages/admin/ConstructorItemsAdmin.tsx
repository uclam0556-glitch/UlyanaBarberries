...
import { Plus, Edit, Trash2, X, Save, Cherry, UploadCloud } from 'lucide-react';
import { useConstructorProducts } from '@/hooks/useProducts';
...
export default function ConstructorItemsAdmin() {
  ...
  const handleSave = async () => {
    if (!formData.name || !formData.price) {
      toast.error('Заполните название и цену ягоды/ингредиента');
      return;
    }

    const saveToast = toast.loading(editingId ? 'Сохранение ингредиента...' : 'Добавление ингредиента...');
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-choco flex items-center gap-2">
            <Cherry className="text-primary w-6 h-6" />
            Ягоды и наполнение конструктора
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Отдельный список ингредиентов (ягоды, посыпки, украшения), которые клиенты выбирают при сборке своего набора
          </p>
        </div>
        <button 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-cream-dark/30 text-xs text-gray-500 uppercase tracking-wider">
                  ...
                  <th className="py-3 px-4 font-semibold">Название ингредиента</th>
                  <th className="py-3 px-4 font-semibold">Цена за шт.</th>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-cream-dark hover:bg-cream/50 transition-colors">
                    <td className="py-3 px-4">
                      {item.images?.[0] ? (
                        <img src={item.images[0]} alt="" className="w-10 h-10 object-cover rounded-lg border border-cream-dark shadow-sm bg-cream" />
                      ) : (
                        <div className="w-10 h-10 bg-cream-dark/30 rounded-lg flex items-center justify-center text-choco text-xs">🍓</div>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <p className="font-semibold text-sm text-choco">{item.name}</p>
                    </td>
                    <td className="py-3 px-4 text-sm font-bold text-primary">{item.price} ₽</td>
                    <td className="py-3 px-4 text-sm text-gray-500">{item.sort_order || 1}</td>
                    <td className="py-3 px-4">
                      {item.is_active ? (
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            >
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-5">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Название ингредиента *</label>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="Например: Клубника в белом шоколаде"
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Цена за 1 шт. (₽) *</label>
                  <input
                    type="number"
                    ...
                    {formData.images?.[0] ? (
                      ...
                      >
                        ...
                        <span className="text-xs font-semibold">
                          {isCompressing ? 'Сжатие...' : 'Загрузить фото ингредиента'}
                        </span>
                      </button>
                    )}
                    <input 
              ...
              <div className="p-4 bg-white border-t border-cream-dark shrink-0 pb-safe">
                ...
                >
                  ...
                  {editingId ? 'Сохранить изменения' : 'Создать ингредиент'}
                </button>
              </div>
            </motion.div>
          </div>
